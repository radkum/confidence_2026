...
extern "system" fn DispatchDeviceControl(_driver: &mut DEVICE_OBJECT, irp: &mut IRP) -> NTSTATUS {
    unsafe {
        ...
        match device_io.IoControlCode {
            ...
            ioctl_code::IOCTL_KILL_PROCESS => {
                // Read PID from METHOD_BUFFERED input buffer
                let input_len = device_io.InputBufferLength;
                if input_len < 4 {
                    return complete_irp_with_status(irp, STATUS_INVALID_BUFFER_SIZE);
                }
                let system_buffer = *irp.AssociatedIrp.SystemBuffer() as *const u32;
                if system_buffer.is_null() {
                    return complete_irp_with_status(irp, STATUS_UNSUCCESSFUL);
                }
                let pid = *system_buffer;
                log::info!("IOCTL_KILL_PROCESS: pid={}", pid);
                // ZwTerminateProcess / PsProcessType not available in km-api-sys;
                // userspace should use Win32 TerminateProcess instead.
            },
            _ => {
...
extern "system" fn DispatchRead(_driver: &mut DEVICE_OBJECT, irp: &mut IRP) -> NTSTATUS {
    unsafe {
        ...
        // copy_events_to_ptr returns 0 in two cases:
        //   1. Queue is empty -- normal, return SUCCESS with 0 bytes
        //   2. Buffer too small for queued events
        // Returning INSUFFICIENT_RESOURCES here breaks userspace polling -- ReadFile
        // fails with Win32 err 1450 even though it's just a poll on empty queue.
        if copied_bytes > 0 {
            log::info!("READ returning {} bytes", copied_bytes);
        }
...
extern "system" fn OnProcessNotify(
    ...
    unsafe {
        ...
        let item = if !create_info.is_null() {
            ...
            let cmd_line = if !create_info.CommandLine.is_null() {
                (*create_info.CommandLine).as_rust_string().unwrap_or_default()
            } else {
            ...
            ProcessCreate {
                ...
                command_line: ItemInfo::string_to_buffer(cmd_line),
            }
        } else {
            ProcessExit {
                pid: HandleToU32!(process_id),
...
// REG_NOTIFY_CLASS values not exported by km-api-sys (only POST_SET_VALUE_KEY = 16).
// Reference: ntddk.h _REG_NOTIFY_CLASS enum (post-operation members).
// IMPORTANT: PRE/POST values are different. 28 = PRE_OPEN_KEY_EX, 29 = POST_OPEN_KEY_EX.
// For PRE the Object isn't created yet; we need POST to inspect the opened key.
const REG_NT_POST_OPEN_KEY: u32 = 13;
const REG_NT_POST_ENUMERATE_KEY: u32 = 20;
const REG_NT_POST_QUERY_KEY: u32 = 22;
const REG_NT_POST_OPEN_KEY_EX: u32 = 29;

/// Returns true if `path` contains "AMSI\Providers" (case-insensitive).
/// The NT path comes as e.g. "\REGISTRY\MACHINE\SOFTWARE\Microsoft\AMSI\Providers".
fn is_amsi_providers_path(path: &str) -> bool {
    let lower = path.to_ascii_lowercase();
    lower.contains("\\amsi\\providers") || lower.ends_with("\\amsi") || lower.contains("\\amsi\\")
}

extern "system" fn OnRegistryNotify(_context: PVOID, arg1: PVOID, arg2: PVOID) -> NTSTATUS {
    let reg_notify = HandleToU32!(arg1);

    // Detect reconnaissance: any read-style operation on AMSI Providers key.
    // Fires when a process opens / enumerates / queries the AMSI registry keys --
    // a strong indicator of AMSI provider hijack preparation (e.g. vtable patch).
    if reg_notify == REG_NT_POST_OPEN_KEY
        || reg_notify == REG_NT_POST_OPEN_KEY_EX
        || reg_notify == REG_NT_POST_ENUMERATE_KEY
        || reg_notify == REG_NT_POST_QUERY_KEY
    {
        unsafe {
            ...
            let status = CmCallbackGetKeyObjectIDEx(&G_COOKIE, op_info.Object, null_mut(), &mut name, 0);
            if !NT_SUCCESS!(status) || name.is_null() {
                return STATUS_SUCCESS;
            }
            if let Some(key_name) = (*name).as_rust_string() {
                if is_amsi_providers_path(&key_name) {
                    let pid = HandleToU32!(PsGetCurrentProcessId());
                    log::info!("PUSH-AMSI pid={} key={}", pid, key_name);
                    let item = ItemInfo::RegistryEnumerate {
                        pid,
            ...
            loop {
                ...
                if key_name.contains(registry_machine) {
                    ...
                    let item = RegistrySetValue {
                        ...
                        tid: HandleToU32!(PsGetCurrentThreadId()),
                        key_name: ItemInfo::string_to_buffer(key_name),
                        value_name: ItemInfo::string_to_buffer(value_name),
...
unsafe fn copy_events_to_ptr(dst_ptr: *mut u8, buffer_size: usize) -> usize {
    let _locker = AutoLock::new(&mut G_MUTEX);
    if let Some(events) = &mut G_EVENTS {
        let item_size = size_of::<ItemInfo>();
        let max_items_in_buf = buffer_size / item_size;
        let items_to_copy = events.len().min(max_items_in_buf);
        if items_to_copy == 0 {
            return 0;
        }
        let copy_bytes = items_to_copy * item_size;

        let events_ptr = events.as_mut_ptr() as *mut u8;
        ptr::copy_nonoverlapping(events_ptr, dst_ptr, copy_bytes);

        // Remove the items we drained; leave the rest for next ReadFile.
        events.drain(..items_to_copy);
        return copy_bytes;
    }
    0
}
