...
fn main() {
    ...
    // Linker flags for kernel-mode driver.
    // Emitted from build.rs (not .cargo/config.toml) so they only apply to THIS
    // crate's cdylib output, not to build scripts of dependencies (winapi etc.)
    // which run as ordinary user-mode binaries.
    let cdylib_link_args = [
        "/NOLOGO",
        "/NXCOMPAT",
        "/NODEFAULTLIB",
        "/SUBSYSTEM:NATIVE",
